...
describe('Bitlab metadata schema compatibility for Pi tools', () => {
  it('widens a strict Edit-like schema with optional Bitlab metadata properties', () => {
    const schema = {
      type: 'object',
    ...
    const widened = allowBitlabMetadataProperties(schema);
  ...
  it('widens the actual Pi Edit tool schema without making metadata required', () => {
    ...
    const widened = allowBitlabMetadataProperties(editTool.parameters);
  ...
  it('returns unknown schema shapes unchanged', () => {
    expect(allowBitlabMetadataProperties(undefined)).toBeUndefined();
    expect(allowBitlabMetadataProperties('schema')).toBe('schema');

    const noProperties = { type: 'string' };
    expect(allowBitlabMetadataProperties(noProperties)).toBe(noProperties);
  });

  it('strips Bitlab metadata before upstream Pi tool execution', () => {
    const input = {
    ...
    const clean = stripBitlabMetadata(input);
  ...
  it('returns the same input object when no metadata is present', () => {
    ...
    expect(stripBitlabMetadata(input)).toBe(input);
  });
});
