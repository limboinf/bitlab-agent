...
/**
 * Regression contract for bitlab-oss#648.
 *
...
describe('applySystemPromptOverride', () => {
  it('stamps state.systemPrompt', () => {
    ...
    applySystemPromptOverride(session as unknown as AgentSession, 'BITLAB_PROMPT');
    expect(session.agent.state.systemPrompt).toBe('BITLAB_PROMPT');
  });

  it('stamps the private _baseSystemPrompt field so session.prompt() reset survives', () => {
    const session = makeFakeSession();
    applySystemPromptOverride(session as unknown as AgentSession, 'BITLAB_PROMPT');
    expect(session._baseSystemPrompt).toBe('BITLAB_PROMPT');
  });
  ...
  it('replaces _rebuildSystemPrompt with a constant function so tool-change rebuilds survive', () => {
    ...
    expect(session._rebuildSystemPrompt!(['read', 'bash'])).toBe('BITLAB_PROMPT');
    expect(session._rebuildSystemPrompt!([])).toBe('BITLAB_PROMPT');
  });
