...
export function isDevRuntime(): boolean {
  ...
  return nodeEnv === 'development' || nodeEnv === 'dev' || getEnv('BITLAB_DEBUG') === '1';
}
...
export function isDeveloperFeedbackEnabled(): boolean {
  const override = parseBooleanEnv(getEnv('BITLAB_FEATURE_DEVELOPER_FEEDBACK'));
  if (override !== undefined) return override;
  return isDevRuntime();
}

/**
 * Runtime-evaluated check for bitlab-cli integration.
 *
 * Defaults to disabled. Override with BITLAB_FEATURE_CLI=1|0.
 */
export function isBitlabCliEnabled(): boolean {
  const override = parseBooleanEnv(getEnv('BITLAB_FEATURE_CLI'));
  if (override !== undefined) return override;
  return false;
}
 ...
 *
 * Defaults to disabled. Override with BITLAB_FEATURE_EMBEDDED_SERVER=1|0.
 */
export function isEmbeddedServerEnabled(): boolean {
  const override = parseBooleanEnv(getEnv('BITLAB_FEATURE_EMBEDDED_SERVER'));
  if (override !== undefined) return override;
  return false;
}
...
export const FEATURE_FLAGS = {
   ...
   * Override with BITLAB_FEATURE_DEVELOPER_FEEDBACK=1|0.
   */
  get developerFeedback(): boolean {
    return isDeveloperFeedbackEnabled();
  },
  /**
   * Enable bitlab CLI guidance and guardrails.
   *
   * Defaults to disabled. Override with BITLAB_FEATURE_CLI=1|0.
   */
  get bitlabCli(): boolean {
    return isBitlabCliEnabled();
  },
  /**
