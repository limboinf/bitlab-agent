...
export const DEBUG = INTERCEPTOR_LOGGING_ENABLED &&
  (process.argv.includes('--debug') || process.env.BITLAB_DEBUG === '1');
...
let _sessionDir: string | null = process.env.BITLAB_SESSION_DIR || null;
 ...
 * - SDK subprocess: BITLAB_SESSION_DIR env var (set by main process before spawn)
