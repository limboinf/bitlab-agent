 ...
 *
 * The `ErrorCode` union is owned by `@bitlab/core` so the wire
...
import type { ErrorCode } from '@bitlab/core/types';
...
const ERROR_DEFINITIONS: Record<ErrorCode, Omit<AgentError, 'code' | 'originalError' | 'details'>> = {
  ...
  model_no_tool_support: {
    ...
    message: 'The selected model does not support tool/function calling, which is required for Bitlab. Please choose a model with tool support (e.g., Claude, GPT-4, Gemini).',
    actions: [
  ...
  sdk_binary_missing: {
    ...
    message:
      'A required Pi runtime file is missing. The app bundle may be incomplete; reinstall Bitlab and retry.',
    actions: [
      { key: 'r', label: 'Retry', action: 'retry' },
    ],
    canRetry: true,
  ...
  if (code === 'model_no_tool_support') {
    ...
    if (modelMatch?.[1]) {
      return {
        code,
        ...definition,
        message: `Model "${modelMatch[1]}" does not support tool/function calling, which is required for Bitlab. Please choose a different model with tool support in Settings.`,
        originalError: errorMessage,
        providerInfo,
      };
    }
