 ...
 *
 * Tests the Pi SDK AgentEvent / AgentSessionEvent → BitlabEvent conversion.
...
describe('PiEventAdapter', () => {
  ...
  describe('message events', () => {
    ...
    it('should forward pi_turn_anchor events as BitlabEvents', () => {
      const events = collect(adapter.adaptEvent({
        type: 'pi_turn_anchor',
        sdkMessageId: 'msg_pi_abc123',
  ...
  describe('sub-turnId isolation', () => {
    ...
    it('should stream reasoning as thinking events on its own sub-turnId', () => {
      collect(adapter.adaptEvent({ type: 'turn_start' } as any));
      ...
      const end = collect(adapter.adaptEvent({
        type: 'message_update',
        assistantMessageEvent: { type: 'thinking_end', content: 'Let me reason.' },
      } as any));
      ...
      expect(first[0].type).toBe('thinking_delta');
      expect(first[0].turnId).toMatch(/__t\d+$/);
      expect(second[0].turnId).toBe(first[0].turnId);
      expect(end[0].type).toBe('thinking_complete');
      expect(end[0].text).toBe('Let me reason.');
      expect(end[0].turnId).toBe(first[0].turnId);
      // Answer text must never share the reasoning message
      expect(text[0].turnId).not.toBe(first[0].turnId);
    });

    it('should emit reasoning from the finished message when it was not streamed', () => {
      collect(adapter.adaptEvent({ type: 'turn_start' } as any));
      ...
      const events = collect(adapter.adaptEvent({
        ...
        message: {
          ...
          stopReason: 'stop',
          content: [
            { type: 'thinking', thinking: 'Reasoned quietly' },
            { type: 'text', text: 'Answer' },
          ],
        },
      } as any));

      // Reasoning comes first so it renders above the answer it produced
      expect(events[0].type).toBe('thinking_complete');
      expect(events[0].text).toBe('Reasoned quietly');
      expect(events[1].type).toBe('text_complete');
      expect(events[1].text).toBe('Answer');
    });

    it('should not repeat streamed reasoning when the message ends', () => {
      collect(adapter.adaptEvent({ type: 'turn_start' } as any));
      collect(adapter.adaptEvent({
        type: 'message_update',
        assistantMessageEvent: { type: 'thinking_delta', delta: 'Reasoned' },
      } as any));
      collect(adapter.adaptEvent({
        type: 'message_update',
        assistantMessageEvent: { type: 'thinking_end', content: 'Reasoned' },
      } as any));
      ...
      expect(events.filter(e => e.type === 'thinking_complete')).toHaveLength(0);
      expect(events[0].type).toBe('text_complete');
    });

    it('should skip redacted reasoning blocks', () => {
      collect(adapter.adaptEvent({ type: 'turn_start' } as any));
    ...
    it('should keep sub-turnIds distinct across Pi turn_end/turn_start inside one user turn', () => {
      // Pi wraps every LLM call in its own turn_start/turn_end, so commentary
      // before a tool call and the final answer arrive in separate Pi turns.
      // They still belong to one user turn and must not share a turnId — the
      // renderer looks messages up by turnId and would overwrite the first one.
      collect(adapter.adaptEvent({ type: 'turn_start' } as any));
      const commentary = collect(adapter.adaptEvent({
        type: 'message_end',
        message: { role: 'assistant', stopReason: 'toolUse', content: 'Let me search first' },
      } as any));
      collect(adapter.adaptEvent({ type: 'turn_end' } as any));

      collect(adapter.adaptEvent({ type: 'turn_start' } as any));
      const final = collect(adapter.adaptEvent({
        type: 'message_end',
        message: { role: 'assistant', stopReason: 'stop', content: 'Here is the answer' },
      } as any));

      expect(commentary[0].isIntermediate).toBe(true);
      expect(final[0].isIntermediate).toBe(false);
      expect(commentary[0].turnId).not.toBe(final[0].turnId);
    });
