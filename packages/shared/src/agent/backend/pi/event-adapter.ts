 ...
 * Bitlab's AgentEvent format for UI compatibility.
 *
...
/**
 * Maps Pi SDK events to BitlabEvents for UI compatibility.
 *
 * Event mapping:
...
export class PiEventAdapter extends BaseEventAdapter {
  ...
  // Sub-turnId for the reasoning block currently streaming, and whether this
  // message already emitted its thinking (so message_end doesn't repeat it)
  private thinkingSubTurnId: string | null = null;
  private emittedThinkingThisMessage: boolean = false;
  ...
  /**
   * Generate a unique sub-turnId for a text block within the current user turn.
   * The counter only resets in onTurnStart(), so every assistant message of one
   * user turn gets a distinct id (m0, m1, m2, …) even across Pi's internal turns.
   */
  ...
  protected onTurnStart(): void {
    ...
    this.emittedThinkingThisMessage = false;
    this.log.debug('Turn started', { turnIndex: this.turnIndex });
  }

  /**
   * Adapt a Pi SDK event to zero or more BitlabEvents.
   */
  *adaptEvent(event: PiEvent): Generator<CraftAgentEvent> {
    ...
    switch (event.type) {
      ...
      case 'turn_end':
        ...
        // Note: subTurnCounter is NOT reset here. Pi emits turn_start/turn_end
        // around every LLM call, so resetting made each assistant message in one
        // user turn reuse `pi-turn-N__m0`. The renderer looks messages up by
        // turnId, so colliding ids let a later text_complete overwrite earlier
        // commentary. Only startTurn() (one user message) resets the counter.
      ...
      case 'message_update': {
        ...
        // Reasoning stream. pi-ai turns every provider's reasoning channel
        // (Anthropic thinking blocks, `reasoning_content` / `reasoning` /
        // `reasoning_text` on OpenAI-compatible endpoints) into these events.
        // Thinking gets its own sub-turn id so it never shares a message with
        // the answer text streaming alongside it.
        ...
        if (amEvent.type === 'thinking_end') {
          ...
          this.thinkingSubTurnId = null;
          this.emittedThinkingThisMessage = true;
          yield {
      ...
      case 'message_end': {
        ...
        // Reasoning fallback: endpoints that don't stream thinking still carry
        // it as a thinking block on the finished message. Emit before the text
        // so the reasoning renders above the answer it produced.
        if (!this.emittedThinkingThisMessage) {
  ...
  /**
   * Extract reasoning text from a Pi AgentMessage.
   * Redacted thinking blocks carry no readable text — skip them rather than
  ...
  private extractThinkingFromMessage(message: unknown): string | null {
    ...
    const msg = message as {
      content?: string | Array<{ type: string; thinking?: string; redacted?: boolean }>;
    };
    if (!Array.isArray(msg.content)) return null;

    const parts = msg.content
      .filter((c) => c.type === 'thinking' && !c.redacted && c.thinking)
      .map((c) => c.thinking!);
