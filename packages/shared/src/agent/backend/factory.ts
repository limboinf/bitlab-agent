 ...
 * Bitlab currently registers PiAgent only.
 *
...
export function createBackend(config: BackendConfig): AgentBackend {
  switch (config.provider) {
    case 'pi':
      ...
      // Auth is API key based via Pi's credential store
...
export async function testBackendConnection(args: {
  ...
  try {
    ...
    const agent = createBackendFromResolvedContext({
      context,
      coreConfig: {
        workspace: { id: '__test', name: 'Connection Test', slug: '__test', kind: 'default', folderPath: null, dataRoot: cwd, createdAt: 0 },
