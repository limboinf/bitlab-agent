...
/**
 * Modes exposed in the composer by default.
 *
 * The UI is a two-state control: "Default permissions" (ask) plus a
 * "Allow full access" switch (allow-all). Explore (safe) is a strict subset of
 * ask — it never prompts because it refuses every write — so surfacing it as a
 * third choice only adds a decision without adding capability. Its rules stay
 * in the engine: `ask` mode uses safe-mode evaluation to decide which calls are
 * mutations worth prompting for (see core/pre-tool-use.ts). Users who want the
 * read-only lock can re-enable it in workspace settings.
 */
export const DEFAULT_CYCLABLE_PERMISSION_MODES: PermissionMode[] = ['ask', 'allow-all'];

/**
 * Cyclable lists that shipped as defaults before the two-state UI. Stored
 * configs carrying one of these were never customized — they just predate the
 * change — so they get collapsed to DEFAULT_CYCLABLE_PERMISSION_MODES on read.
 * A list a user actually picked will not match one of these exactly, and can
 * still be edited in workspace settings.
 */
const LEGACY_CYCLABLE_DEFAULTS: PermissionMode[][] = [
  ['safe', 'ask', 'allow-all'],
  ['safe', 'allow-all'],
];

/**
 * Whether a stored cyclable-mode list is an untouched pre-simplification default.
 */
export function isLegacyCyclableDefault(modes: PermissionMode[]): boolean {
  return LEGACY_CYCLABLE_DEFAULTS.some(
    legacy => legacy.length === modes.length && legacy.every((mode, index) => modes[index] === mode),
  );
}

/**
 * Keep the starting mode consistent with what the composer can actually show.
 * Workspaces created before the two-state UI default to 'safe'; once Explore is
 * off the menu, that would strand a session in a mode with no control for it.
 */
export function resolveStartingPermissionMode(
  mode: PermissionMode | undefined,
  cyclableModes: PermissionMode[],
): PermissionMode {
  if (mode && cyclableModes.includes(mode)) return mode;
  return cyclableModes.find(candidate => candidate !== 'allow-all') ?? 'ask';
}
 ...
 *
 * The actual patterns are loaded from ~/.bitlab/permissions/default.json
 * at runtime by PermissionsConfigCache. This fallback ensures the app works
 * even if the JSON file is missing or invalid.
 *
 * To customize allowed commands, edit ~/.bitlab/permissions/default.json
 */
...
/**
 * i18n keys for the composer's permission control.
 *
 * PERMISSION_MODE_CONFIG carries English-only strings aimed at the agent side
 * (prompts, logs). These keys are the user-facing half, phrased as states
 * ("Default permissions" / "Full access") rather than modes, so the control
 * reads as a situation the user is in rather than a taxonomy to learn.
 */
export const PERMISSION_MODE_LABEL_KEYS: Record<PermissionMode, { label: string; description: string }> = {
  'safe': { label: 'mode.explore', description: 'mode.exploreFullDesc' },
  'ask': { label: 'mode.defaultPermissions', description: 'mode.defaultPermissionsDesc' },
  'allow-all': { label: 'mode.fullAccess', description: 'mode.fullAccessDesc' },
};
...
export const PERMISSION_MODE_CONFIG: Record<PermissionMode, {
  ...
  'ask': {
    ...
    // Circle-check icon from Lucide — the default, guarded state
    svgPath: 'M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20z M8.5 12.5l2.5 2.5 4.5-5',
    colorClass: {
      // Neutral on purpose: the default state should recede, so the
      // full-access state is the only one that visually raises a hand.
