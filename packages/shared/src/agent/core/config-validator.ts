...
/**
 * Bitlab specific config files that have known schemas.
 */
const BITLAB_AGENT_CONFIG_PATTERNS = [
  // Main config
  /\.bitlab\/config\.json$/,
  // Preferences
  /\.bitlab\/preferences\.json$/,
  // Permissions
  /\.bitlab\/workspaces\/[^/]+\/permissions\.json$/,
  /\.bitlab\/permissions\/[^/]+\.json$/,
  // Theme
  /\.bitlab\/workspaces\/[^/]+\/theme\.json$/,
  // Tool icons
  /\.bitlab\/tool-icons\/tool-icons\.json$/,
];
...
function isBitlabConfigPath(filePath: string): boolean {
...
export class ConfigValidator {
  ...
  /**
   * Check if a file path is a Bitlab config file.
   *
   * @param filePath - Path to check
   * @returns true if this is a Bitlab config
   */
  isCraftAgentConfig(filePath: string): boolean {
    const normalizedPath = normalizeConfigPathForMatch(filePath);
    return isBitlabConfigPath(normalizedPath) || BITLAB_AGENT_CONFIG_PATTERNS.some((pattern) => pattern.test(normalizedPath));
  }
