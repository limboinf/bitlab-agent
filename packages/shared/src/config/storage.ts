...
import type { StoredAttachment, StoredMessage } from '@bitlab/core/types';
...
const FALLBACK_CONFIG_DEFAULTS: ConfigDefaults = {
  version: '1.0',
  description: 'Default configuration values for Bitlab',
  defaults: {
...
/**
 * Load config defaults from ~/.bitlab/config-defaults.json
...
export function loadConfigDefaults(): ConfigDefaults {
  ...
  defaults.workspaceDefaults.cyclablePermissionModes =
    normalizedCyclable.length >= 2 && !isLegacyCyclableDefault(normalizedCyclable)
      ? normalizedCyclable
      : [...DEFAULT_CYCLABLE_PERMISSION_MODES];

  defaults.workspaceDefaults.permissionMode = resolveStartingPermissionMode(
    parsedPermissionMode ?? undefined,
    defaults.workspaceDefaults.cyclablePermissionModes,
  );
...
export function ensureConfigDir(): void {
  ...
  backupConfigFile();
  // Initialize bundled docs (creates ~/.bitlab/docs/ with sources.md, agents.md, permissions.md)
  initializeDocs();
...
export function loadStoredConfig(): StoredConfig | null {
  try {
    ...
    // Expand project folder paths for runtime use. Workspace data roots are
    // derived from stable slugs and are never read from user-controlled paths.
    for (const workspace of config.workspaces) {
      if (workspace.folderPath) workspace.folderPath = expandPath(workspace.folderPath);
    }
    ...
    for (const workspace of config.workspaces) {
      const dataRoot = getWorkspaceDataRoot(workspace.slug);
      if (!isValidWorkspace(dataRoot)) {
        try {
          createWorkspaceAtPath(dataRoot, workspace.name, undefined, workspace);
        } catch (wsError) {
          debug('[config] Failed to create workspace at', dataRoot, ':', wsError instanceof Error ? wsError.message : wsError);
        }
      }
    }
...
export function saveConfig(config: StoredConfig): void {
  ...
  const storageConfig: StoredConfig = {
    ...
    workspaces: config.workspaces.map(ws => ({
      ...ws,
      folderPath: ws.folderPath ? toPortablePath(ws.folderPath) : null,
    })),
  };
 ...
 * Used to set BITLAB_GIT_BASH_PATH for subprocess tools.
 */
 ...
 */
export function findWorkspaceIcon(dataRoot: string): string | null {
  return findIconFile(dataRoot) ?? null;
}
...
export function getWorkspaces(): Workspace[] {
  ...
  return workspaces.map(w => {
    const dataRoot = getWorkspaceDataRoot(w.slug);
    const wsConfig = loadWorkspaceConfig(dataRoot);
    const name = wsConfig?.name || w.name || 'Untitled';
    ...
    if (!iconUrl || (!iconUrl.startsWith('http://') && !iconUrl.startsWith('https://'))) {
      const localIcon = findWorkspaceIcon(dataRoot);
      if (localIcon) {
    ...
    return { ...w, name, dataRoot, iconUrl };
  });
}
...
export function getActiveWorkspace(): Workspace | null {
  ...
  if (!config.activeWorkspaceId) return workspaces[0] ?? null;
  return workspaces.find(w => w.id === config.activeWorkspaceId) ?? workspaces[0] ?? null;
}
...
export async function switchWorkspaceAtomic(workspaceId: string): Promise<{ workspace: Workspace; session: SessionConfig } | null> {
  ...
  const session = await getOrCreateLatestSession(getWorkspaceDataRoot(workspace.slug));
  ...
  return { workspace: { ...workspace, dataRoot: getWorkspaceDataRoot(workspace.slug) }, session };
}

/**
 * Add a workspace to the global config.
 * Register a user folder as a workspace. The folder is the user-facing
 * identity; Bitlab-owned data is always stored under CONFIG_DIR/workspaces.
 */
export function addWorkspace(input: {
  ...
  if (kind === 'folder' && !input.folderPath) {
    throw new Error('Folder workspace requires a project folder path');
  }
  if (kind === 'default' && input.folderPath !== null) {
    throw new Error('Default workspace cannot have a project folder path');
  }

  const folderPath = input.folderPath
    ? (existsSync(input.folderPath) ? realpathSync(input.folderPath) : input.folderPath)
    : null;

  const existing = config.workspaces.find(w => w.kind === kind && w.folderPath === folderPath);
  if (existing) {
    ...
    // Keep the private workspace metadata aligned with the registry. This is
    // also what makes re-opening an existing folder with a new display name
    // deterministic across restarts.
    const dataRoot = getWorkspaceDataRoot(updated.slug);
    const localConfig = loadWorkspaceConfig(dataRoot);
    if (localConfig && localConfig.name !== updated.name) {
      localConfig.name = updated.name;
      saveWorkspaceConfig(dataRoot, localConfig);
    }

    saveConfig(config);
    return { ...updated, dataRoot };
  }

  const id = kind === 'default' ? 'default' : generateWorkspaceId();
  const baseSlug = kind === 'default'
    ? 'default'
    : generateSlug(basename(folderPath!));
  const baseTaken = kind !== 'default' && (
    baseSlug === 'default'
    || config.workspaces.some(workspace => workspace.slug === baseSlug)
    || existsSync(getWorkspaceDataRoot(baseSlug))
  );
  const suffix = folderPath
    ? createHash('sha256').update(folderPath).digest('hex').slice(0, 8)
    : id.slice(0, 8);
  const slug = baseTaken ? `${baseSlug}-${suffix}` : baseSlug;
  ...
  const dataRoot = getWorkspaceDataRoot(slug);

  if (!isValidWorkspace(dataRoot)) {
    createWorkspaceAtPath(dataRoot, stored.name, undefined, stored);
  }
  ...
  if (config.workspaces.length === 1) {
    config.activeWorkspaceId = stored.id;
  }

  saveConfig(config);
  return { ...stored, dataRoot };
}
...
export async function removeWorkspace(workspaceId: string): Promise<boolean> {
  ...
  if (config.workspaces[index]!.kind === 'default') return false;
  ...
  const workspaceDataDir = getWorkspaceDataRoot(removed!.slug);
...
/**
 * Rename a workspace through the registry seam so both persisted records stay
 * aligned. Callers should not write the private config name directly.
 */
export function renameWorkspace(workspaceId: string, nextName: string): Workspace {
  ...
  const dataRoot = getWorkspaceDataRoot(workspace.slug);
  const localConfig = loadWorkspaceConfig(dataRoot);
  if (!localConfig) throw new Error(`Workspace data not found: ${dataRoot}`);
...
export type { StoredAttachment, StoredMessage } from '@bitlab/core/types';
...
/**
 * Get the path to the app-level theme override file (~/.bitlab/theme.json).
 */
 ...
 * Preset themes are stored at ~/.bitlab/themes/
 */
...
export function migrateOrphanedDefaultConnections(): void {
  ...
  try {
    ...
    for (const ws of workspaces) {
      const wsConfig = loadWorkspaceConfig(ws.dataRoot);
      if (wsConfig?.defaults?.defaultLlmConnection) {
        ...
        if (!exists) {
          ...
          saveWorkspaceConfig(ws.dataRoot, wsConfig);
        }
      }
    }
  } catch (error) {
...
/**
 * Returns the path to the tool-icons directory: ~/.bitlab/tool-icons/
 */
