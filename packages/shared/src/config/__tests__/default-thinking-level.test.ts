...
function setupWorkspaceConfigDir() {
  const configDir = mkdtempSync(join(tmpdir(), 'bitlab-config-thinking-'))
  ...
  writeFileSync(
    configPath,
    JSON.stringify({
      workspaces: [{ id: 'ws-1', name: 'My Workspace', slug: 'my-workspace', kind: 'folder', folderPath: workspaceRoot, createdAt: Date.now() }],
