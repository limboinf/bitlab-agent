#!/usr/bin/env bun
/**
 * Debug script to print the full Bitlab system prompt with annotations.
...
${colors.bgMagenta}${colors.bold}                      BITLAB SYSTEM PROMPT BREAKDOWN                            ${colors.reset}
${colors.bgMagenta}${colors.bold}                                                                                ${colors.reset}
`);
...
printAnnotation('  2. Bitlab Environment Marker - version, platform, arch');
...
const systemPrompt = getSystemPrompt(
  ...
  '/Users/example/.bitlab/workspaces/abc123' // Example workspace path
);
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@bitlab/electron/main.log' },
  '/Users/example/.bitlab/workspaces/abc123'
);
...
const sessionState = formatSessionState('260121-example-session', {
  plansFolderPath:
    '/Users/example/.bitlab/workspaces/abc123/sessions/260121-example-session/plans',
});
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// Bitlab additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. Bitlab Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
...
${colors.bold}Key Files:${colors.reset}
  ...
  packages/shared/src/agent/bitlab.ts       ${colors.dim}// User message building${colors.reset}
