 ...
 * - 'default': Full Bitlab system prompt
...
export function getMiniAgentSystemPrompt(workspaceRootPath?: string): string {
  ...
  return `You are a focused assistant for quick configuration edits in Bitlab.
...
export function getSystemPrompt(
  ...
  const basePrompt = getBitlabAssistantPrompt(workspaceRootPath, backendName);
...
/**
 * Get the Bitlab environment marker for SDK JSONL detection.
 * This marker is embedded in the system prompt and allows us to identify
 * Bitlab sessions in exported JSONL data.
 */
function getBitlabEnvironmentMarker(): string {
  const platform = process.platform; // 'darwin', 'win32', 'linux'
  const arch = process.arch; // 'arm64', 'x64'
  const osVersion = os.release(); // OS kernel version

  return `<bitlab_environment version="${APP_VERSION}" platform="${platform}" arch="${arch}" os_version="${osVersion}" />`;
}

/**
 * Get the Bitlab system prompt with workspace-specific paths.
 *
 ...
 * @param backendName - Backend name for "powered by X" text (default: 'Claude Code')
 */
function getBitlabAssistantPrompt(workspaceRootPath?: string, backendName: string = 'Pi'): string {
  // Default to ${APP_ROOT}/workspaces/{id} if no path provided
  const workspacePath = workspaceRootPath || `${APP_ROOT}/workspaces/{id}`;

  const environmentMarker = getBitlabEnvironmentMarker();
...
You are Bitlab - a desktop AI assistant powered by ${backendName}. You can reason, write and execute code, work with files and documents, browse the web, and use approved tools.
...
| LLM Tool | \`${DOC_REFS.llmTool}\` | When using \`call_llm\` for subtasks |${FEATURE_FLAGS.bitlabCli ? `
| Bitlab CLI | \`${DOC_REFS.bitlabCli}\` | When managing local workspaces and sessions |` : ''}

**IMPORTANT:** Always read the relevant doc file BEFORE making changes. Do NOT guess schemas - these have specific patterns that differ from standard approaches.${FEATURE_FLAGS.bitlabCli ? `

## Bitlab CLI

Use \`bitlab\` for local workspace, session, connection, and configuration operations.

- CLI help: \`bitlab --help\`
- Canonical reference: \`${DOC_REFS.bitlabCli}\`` : ''}
...
!!IMPORTANT!!. You must refer to yourself as Bitlab when asked. You can acknowledge that you are powered by ${backendName}.
...
You have a \`send_developer_feedback\` tool — a direct line to the Bitlab development team.
