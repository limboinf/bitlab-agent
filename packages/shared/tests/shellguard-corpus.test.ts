...
const TEST_MODE_CONFIG = {
  ...
  readOnlyBashPatterns: [
    ...
    // bitlab CLI read-only
    ...getBitlabReadOnlyBashPatterns().map(rule => ({
...
// ============================================================
// Group 27: bitlab CLI allowlist
// ============================================================

describe('ShellGuard corpus: bitlab CLI allowlist', () => {
  const shouldAllow = [
    'bitlab',
    'bitlab --help',
    'bitlab --version',
    'bitlab --discover',
    'bitlab workspace --help',
    'bitlab workspace list',
    'bitlab session list',
    'bitlab session messages abc123',
    'bitlab connections list',
    'bitlab config validate',
  ];

  const shouldBlock = [
    'bitlab label create --name Bug',
    'bitlab label update bug --name "Bug Report"',
    'bitlab label delete bug',
    'bitlab label move bug --parent root',
    'bitlab label reorder --parent root a b c',
    'bitlab source create --name Linear --provider linear --type mcp',
    'bitlab source update linear --json "{\"enabled\":false}"',
    'bitlab source delete linear',
    'bitlab skill create --name "Review" --description "x"',
    'bitlab skill update review --json "{\"description\":\"y\"}"',
    'bitlab skill delete review',
    'bitlab skill list',
    'bitlab session delete abc123',
    'bitlab connections test openai',
    'bitlab workspace create Example',
  ];
