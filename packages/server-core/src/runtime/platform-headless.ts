...
function createConsoleLogger(): Logger {
  ...
  return {
    ...
    debug: (...args) => {
      if (process.env.BITLAB_DEBUG === 'true' || process.env.BITLAB_IS_PACKAGED !== 'true') {
 ...
 * - BITLAB_APP_ROOT — override appRootPath (default: cwd)
 * - BITLAB_RESOURCES_PATH — override resourcesPath (default: cwd/resources)
 * - BITLAB_IS_PACKAGED — 'true' for production (default: false)
 * - BITLAB_VERSION — app version string (default: '0.0.0-dev')
 * - BITLAB_DEBUG — 'true' to enable debug logging
 */
export function createHeadlessPlatform(options?: { appVersion?: string }): PlatformServices {
  const logger = createConsoleLogger()
  const isDebugMode = process.env.BITLAB_DEBUG === 'true' || process.env.BITLAB_IS_PACKAGED !== 'true'

  return {
    appRootPath: process.env.BITLAB_APP_ROOT || process.cwd(),
    resourcesPath: process.env.BITLAB_RESOURCES_PATH || join(process.cwd(), 'resources'),
    isPackaged: process.env.BITLAB_IS_PACKAGED === 'true',
    appVersion: process.env.BITLAB_VERSION || options?.appVersion || '0.0.0-dev',
