...
import { getSessionFilePath } from '@bitlab/shared/sessions/storage'
...
describe('sendMessage durability', () => {
  ...
  it('uses the first-message preview as a fallback for legacy unnamed sessions', () => {
    const managed = createManagedSession(
      ...
      { id: 'ws_test', name: 'Test Workspace', slug: 'test-workspace', kind: 'folder', folderPath: tmpRoot, dataRoot: tmpRoot, createdAt: Date.now() } as never,
    )
