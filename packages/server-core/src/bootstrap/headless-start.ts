...
import { ensureConfigDir, loadStoredConfig, saveConfig } from '@bitlab/shared/config'
import { CONFIG_DIR } from '@bitlab/shared/config/paths'
...
export interface ServerBootstrapOptions<TSessionManager, THandlerDeps> {
  ...
  /** Accept loopback handshakes without auth (local WebUI convenience). */
        ...
        if (lock.pid === process.pid) {
          ...
          if (isLockFromPreviousBoot(lock.startedAt)) {
            ...
            throw new Error(
              ...
              `To run a parallel instance (e.g. for dev), set BITLAB_CONFIG_DIR to a different path.`
            )
          }
        } else {
...
export async function bootstrapServer<TSessionManager, THandlerDeps>(
  ...
  const serverToken = options.serverToken ?? process.env.BITLAB_SERVER_TOKEN
  if (!serverToken) {
    throw new Error('Server token is required. Pass options.serverToken or set BITLAB_SERVER_TOKEN.')
  }
  ...
  const bundledAssetsRoot = options.bundledAssetsRoot
    ?? process.env.BITLAB_BUNDLED_ASSETS_ROOT
    ?? process.cwd()
  ...
  const rpcHost = options.rpcHost ?? process.env.BITLAB_RPC_HOST ?? '127.0.0.1'
  const rpcPortRaw = options.rpcPort ?? parseInt(process.env.BITLAB_RPC_PORT ?? '9100', 10)
  ...
  const wsServer = new WsRpcServer({
    ...
    allowLoopbackWithoutAuth: options.allowLoopbackWithoutAuth,
  ...
  platform.logger.info(`Bitlab server listening on ${wsServer.protocol}://${rpcHost}:${wsServer.port}`)
