...
import { getWorkspaceByNameOrId, type Workspace } from '@bitlab/shared/config'
...
export function getWorkspaceAllowedDirs(workspaceId?: string | null): string[] {
  ...
  const dirs: string[] = [workspace.dataRoot]
  if (workspace.folderPath) dirs.push(workspace.folderPath)
  return dirs
}
