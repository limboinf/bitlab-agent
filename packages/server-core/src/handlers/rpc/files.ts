...
import { resizeImageForAPI, inspectImageBuffer } from '@bitlab/server-core/services'
import { sanitizeFilename, validateFilePath, getWorkspaceAllowedDirs } from '@bitlab/server-core/handlers'
...
export function registerFilesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.file.STORE_ATTACHMENT, async (ctx, sessionId: string, attachment: FileAttachment): Promise<StoredAttachment> => {
    ...
    try {
      ...
      const workspaceRootPath = workspace.dataRoot
