...
import { CLIENT_OPEN_EXTERNAL } from '@bitlab/server-core/transport'
import type { RpcServer, HandlerFn, RequestContext } from '@bitlab/server-core/transport'
...
describe('registerSystemCoreHandlers OPEN_URL', () => {
  it('routes bitlab action links internally via deeplink:navigate', async () => {
    const { openUrl, ctx, invokeClientCalls, pushCalls } = createTestHarness()

    await openUrl(ctx, 'bitlab://action/new-session?input=sg&send=true')
  ...
  it('routes workspace deep links to workspace target when URL workspace differs', async () => {
    ...
    await openUrl(ctx, 'bitlab://workspace/ws-2/action/new-session?input=hello')
  ...
  it('falls back to client openExternal for bitlab window-mode links', async () => {
    const { openUrl, ctx, invokeClientCalls, pushCalls } = createTestHarness()

    await openUrl(ctx, 'bitlab://action/new-session?window=focused')
    ...
    expect(invokeClientCalls[0]).toEqual({
      ...
      args: ['bitlab://action/new-session?window=focused'],
    })
  })
