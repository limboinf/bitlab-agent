...
import { getWorkspaceByNameOrId, addWorkspace, removeWorkspace, setActiveWorkspace } from '@bitlab/shared/config'
import { perf } from '@bitlab/shared/utils'
import { safeJsonParse } from '@bitlab/shared/utils/files'
...
export function registerWorkspaceCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  // Get workspaces (LOCAL_ONLY — includes local folderPath for Electron).
  ...
  server.handle(RPC_CHANNELS.workspaces.CREATE, async (_ctx, folderPath: string, name: string) => {
    const projectPath = folderPath.trim()
    const validation = isValidWorkspaceRootPath(projectPath)
    if (!validation.valid) {
      throw new Error(validation.reason!)
    }
    if (!existsSync(projectPath)) mkdirSync(projectPath, { recursive: true })

    const workspace = addWorkspace({ name, folderPath: projectPath })
    // Make it active
    setActiveWorkspace(workspace.id)
    deps.platform.logger.info(`Created workspace "${name}" for ${projectPath}`)
    return workspace
  })
  ...
  server.handle(RPC_CHANNELS.workspace.GET_PERMISSIONS, async (_ctx, workspaceId: string) => {
    ...
    const { getWorkspacePermissionsPath } = await import('@bitlab/shared/agent')
    const path = getWorkspacePermissionsPath(workspace.dataRoot)
  ...
  server.handle(RPC_CHANNELS.permissions.GET_DEFAULTS, async () => {
    const { existsSync, readFileSync } = await import('fs')
    const { getAppPermissionsDir } = await import('@bitlab/shared/agent')
  ...
  server.handle(RPC_CHANNELS.workspace.READ_IMAGE, async (_ctx, workspaceId: string, relativePath: string) => {
    ...
    const absolutePath = normalize(join(workspace.dataRoot, relativePath))

    // Double-check the resolved path is still within workspace
    if (!absolutePath.startsWith(workspace.dataRoot)) {
  ...
  server.handle(RPC_CHANNELS.workspace.WRITE_IMAGE, async (_ctx, workspaceId: string, relativePath: string, base64: string, mimeType: string) => {
    ...
    if (fileName.startsWith('icon.')) {
      const files = readdirSync(workspace.dataRoot)
      for (const file of files) {
        if (file.startsWith('icon.') && file !== fileName) {
          const oldPath = join(workspace.dataRoot, file)
          try {
            unlinkSync(oldPath)
          } catch {
  ...
  server.handle(RPC_CHANNELS.theme.GET_WORKSPACE_COLOR_THEME, async (_ctx, workspaceId: string) => {
    const { getWorkspaces } = await import('@bitlab/shared/config/storage')
    const { getWorkspaceColorTheme } = await import('@bitlab/shared/workspaces/storage')
    const workspaces = getWorkspaces()
    const workspace = workspaces.find(w => w.id === workspaceId)
    if (!workspace) return null
    return getWorkspaceColorTheme(workspace.dataRoot) ?? null
  })

  server.handle(RPC_CHANNELS.theme.SET_WORKSPACE_COLOR_THEME, async (_ctx, workspaceId: string, themeId: string | null) => {
    const { getWorkspaces } = await import('@bitlab/shared/config/storage')
    const { setWorkspaceColorTheme } = await import('@bitlab/shared/workspaces/storage')
    const workspaces = getWorkspaces()
    const workspace = workspaces.find(w => w.id === workspaceId)
    if (!workspace) return
    setWorkspaceColorTheme(workspace.dataRoot, themeId ?? undefined)
  })
  ...
  server.handle(RPC_CHANNELS.theme.GET_ALL_WORKSPACE_THEMES, async () => {
    ...
    for (const ws of workspaces) {
      themes[ws.id] = getWorkspaceColorTheme(ws.dataRoot)
    }
    return themes
  })
  ...
  server.handle(RPC_CHANNELS.toolIcons.GET_MAPPINGS, async () => {
    const { getToolIconsDir } = await import('@bitlab/shared/config/storage')
    const { loadToolIconConfig } = await import('@bitlab/shared/utils/cli-icon-resolver')
    const { encodeIconToDataUrl } = await import('@bitlab/shared/utils/icon-encoder')
    const { join } = await import('path')
  ...
  server.handle(RPC_CHANNELS.logo.GET_URL, async (_ctx, serviceUrl: string, provider?: string) => {
    const { getLogoUrl } = await import('@bitlab/shared/utils/logo')
