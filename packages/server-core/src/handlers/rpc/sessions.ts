...
import { RPC_CHANNELS, type FileAttachment, type SendMessageOptions, type SessionEvent } from '@bitlab/shared/protocol'
...
import { isValidThinkingLevel, THINKING_LEVEL_IDS } from '@bitlab/shared/agent/thinking-levels'
...
async function scanSessionDirectory(dirPath: string): Promise<import('@bitlab/shared/protocol').SessionFile[]> {
  ...
  const files: import('@bitlab/shared/protocol').SessionFile[] = []
...
export function registerSessionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.sessions.CREATE, async (_ctx, workspaceId: string, options?: import('@bitlab/shared/protocol').CreateSessionOptions) => {
  ...
  server.handle(RPC_CHANNELS.sessions.COMMAND, async (
    _ctx,
    sessionId: string,
    command: import('@bitlab/shared/protocol').SessionCommand
  ) => {
    switch (command.type) {
      case 'flag':
  ...
  server.handle(RPC_CHANNELS.sessions.SEARCH_CONTENT, async (_ctx, workspaceId: string, query: string, searchId?: string) => {
    ...
    const { searchSessions } = await import('@bitlab/server-core/services')
    const { getWorkspaceSessionsPath } = await import('@bitlab/shared/workspaces')

    const sessionsDir = getWorkspaceSessionsPath(workspace.dataRoot)
  ...
  const importHandler = async (_ctx: any, targetWorkspaceId: string, bundle: unknown, mode: string) => {
    ...
    return sessionManager.importSession(targetWorkspaceId, bundle as import('@bitlab/shared/sessions').SessionBundle, mode)
  }
