...
import { getWorkspaceByNameOrId, getGitBashPath, setGitBashPath, clearGitBashPath } from '@bitlab/shared/config'
import { classifyExternalUrl, formatBlockedUrlError } from '@bitlab/shared/utils/url-safety'
import { isUsableGitBashPath, validateGitBashPath } from '@bitlab/server-core/services'
...
function parseInternalBitlabDeepLink(parsed: URL): ParsedInternalDeepLink | null {
  if (parsed.protocol !== 'bitlab:') return null
...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.gitbash.CHECK, async () => {
    ...
    if (persistedPath) {
      if (await isUsableGitBashPath(persistedPath)) {
        process.env.BITLAB_GIT_BASH_PATH = persistedPath.trim()
    ...
    for (const bashPath of commonPaths) {
      if (await isUsableGitBashPath(bashPath)) {
        process.env.BITLAB_GIT_BASH_PATH = bashPath
    ...
    try {
      ...
      if (firstPath && firstPath.toLowerCase().includes('git') && await isUsableGitBashPath(firstPath)) {
        process.env.BITLAB_GIT_BASH_PATH = firstPath
    ...
    delete process.env.BITLAB_GIT_BASH_PATH
  ...
  server.handle(RPC_CHANNELS.gitbash.SET_PATH, async (_ctx, bashPath: string) => {
    ...
    process.env.BITLAB_GIT_BASH_PATH = validation.path
    return { success: true }
  })
  ...
  // Shell operations - open URL in external browser (or handle bitlab:// internally)
  server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url: string) => {
    ...
    try {
      ...
      if (classification.kind === 'internal-deeplink') {
        const deepLink = parseInternalBitlabDeepLink(parsed)
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing bitlab:// URL internally via deeplink:navigate')
        ...
        deps.platform.logger.info('[OPEN_URL] Falling back to client openExternal for bitlab:// URL')
