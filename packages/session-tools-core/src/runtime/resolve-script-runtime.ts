...
export interface ResolveScriptRuntimeContext {
  /**
   * Whether host app is packaged. Defaults to BITLAB_IS_PACKAGED=1.
...
function inferPackagedMode(ctx?: ResolveScriptRuntimeContext): boolean {
  ...
  return process.env.BITLAB_IS_PACKAGED === '1';
}
...
function resolveResourcesBase(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.resourcesBasePath || process.env.BITLAB_RESOURCES_BASE;
...
function resolveAppRoot(ctx?: ResolveScriptRuntimeContext): string | null {
  const explicit = ctx?.appRootPath || process.env.BITLAB_APP_ROOT;
...
function validatePackagedEnvRuntime(command: string, label: string): string {
  ...
  if (!isAbsolute(command) && !hasPathSeparator) {
    throw new Error(
      ...
      'Packaged builds do not allow PATH-based runtime resolution. Configure an absolute BITLAB_* path or ship a bundled runtime.'
    );
  }
  ...
  if (!existsSync(resolvedCommand)) {
    throw new Error(
      ...
      'Configure a valid absolute BITLAB_* path or ship a bundled runtime.'
    );
  }
 ...
 *
 * Resolution order:
 * - env override (BITLAB_UV / BITLAB_NODE / BITLAB_BUN)
...
export function resolveScriptRuntime(
  ...
  if (language === 'python3') {
    if (process.env.BITLAB_UV) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.BITLAB_UV, 'Python/uv')
        : process.env.BITLAB_UV;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Python runtime unavailable: uv was not found. Configure BITLAB_UV or install uv on PATH.'
    );
  }

  if (language === 'node') {
    if (process.env.BITLAB_NODE) {
      const cmd = isPackaged
        ? validatePackagedEnvRuntime(process.env.BITLAB_NODE, 'Node')
        : process.env.BITLAB_NODE;
    ...
    throw new Error(
      isPackaged
        ...
        : 'Node runtime unavailable: configure BITLAB_NODE or install node on PATH.'
    );
  }

  if (process.env.BITLAB_BUN) {
    const cmd = isPackaged
      ? validatePackagedEnvRuntime(process.env.BITLAB_BUN, 'Bun')
      : process.env.BITLAB_BUN;
    return { command: cmd, argsPrefix: [], source: 'env' };
  }
  ...
  throw new Error(
    isPackaged
      ...
      : 'Bun runtime unavailable: configure BITLAB_BUN or install bun on PATH.'
  );
}
