...
export interface PlatformActions {
  ...
  /**
   * Hand a local HTML file to the OS so it opens in the user's browser
   * (Electron: shell.openPath, i.e. the system file association).
   *
   * Lets an agent-authored page run for real — own origin, sibling assets, no
   * sandbox — which the in-app preview deliberately withholds. Note this
   * follows the file association: if the user mapped .html to an editor, that
   * is what opens. Forcing a browser would mean shell.openExternal on a file://
   * URL, which is blocked repo-wide (it can launch local executables on
   * Windows — see url-safety.ts).
   *
   * Web: not available (buttons hide when undefined) — the path is local to the
   * machine running the agent, not to the browser.
   */
  onOpenFileInBrowser?: (path: string) => void

  /**
   * Build a URL the HTML preview iframe can load a local file from
   * (Electron: the html-preview:// scheme).
   *
   * Loading from a URL rather than srcDoc is what lets a previewed page use
   * third-party scripts, styles and fonts: a srcDoc iframe inherits the app's
   * CSP, which blocks them. It also makes the page's relative paths resolve
   * against its own directory.
   *
   * Web: not available (preview falls back to inline srcDoc).
   */
  getHtmlPreviewUrl?: (path: string) => string
