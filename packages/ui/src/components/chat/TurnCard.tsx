...
/**
 * Tail of a live reasoning stream.
 *
 * Reasoning runs long and the row is a single truncated line, so showing the
 * head would freeze on the first sentence for the whole stream. The tail keeps
 * the row moving with the model.
 */
function getStreamingTail(text: string, maxChars = 120): string {
  const flat = text.replace(/\s+/g, ' ').trim()
  return flat.length > maxChars ? `…${flat.slice(-maxChars)}` : flat
}
...
/**
 * Repaint interval for streaming response text.
 *
 * Caps how often the markdown renderer re-runs while deltas arrive; low enough
 * that the text reads as continuous growth, high enough that a fast token
 * stream does not re-render per token.
 */
const CONTENT_THROTTLE_MS = 50

/**
 * Rough character budget of one collapsed activity row.
 *
 * Only used to decide whether expanding a reasoning/commentary block would
 * reveal anything the truncated summary does not already show.
 */
const SUMMARY_ROW_MAX_CHARS = 80
...
interface ActivityRowProps {
  ...
  onOpenFile?: (path: string) => void
  /** Open a URL from rendered markdown in an expanded reasoning/commentary row */
  onOpenUrl?: (url: string) => void
}
...
/**
 * Reasoning and commentary rows.
 *
 * Both are free-form model prose, so they share one row type: a truncated
 * summary line that expands in place. Reading a thought should not cost a
 * full-screen overlay — the corner button still opens one for the long tail.
 */
function TextActivityRow({
  activity,
  kind,
  onOpenDetails,
  isLastChild,
  ...
  return (
    <div className="flex items-stretch">
      ...
      <div className="flex-1 min-w-0">
        <div
          className={cn(
            'group/row flex items-center gap-2 py-0.5 min-w-0',
            toneClass,
        ...
        >
          {isStreaming ? (
            ...
            <Icon className={cn(SIZE_CONFIG.iconSize, 'shrink-0')} />
          )}
          ...
          <span
            className={cn(
              'truncate flex-1',
              isReasoning && 'italic',
              canExpand && 'group-hover/row:underline'
            )}
          >
            {summary}
          </span>
          ...
          {canExpand && (
            <motion.div
              ...
              className={cn(SIZE_CONFIG.iconSize, 'flex items-center justify-center shrink-0 opacity-50')}
            >
              <ChevronRight className={SIZE_CONFIG.iconSize} />
            </motion.div>
          )}

          {/* Escape hatch to the full-screen overlay, for very long blocks */}
          {onOpenDetails && settled && (
            <div
              role="button"
              tabIndex={0}
              onClick={(e) => {
...
/**
 * The unfolded body of a tool row: the edit it made, or the output it printed.
 *
 * Long results are cut off here rather than scrolled — past a certain length
 * the overlay is the better reader, and the cut doubles as the link to it.
 */
function InlineToolDetail({ detail, onOpenDetails }: {
  detail: InlineToolDetail
  onOpenDetails?: () => void
}) {
  const hidden = detail.lineCount - INLINE_DETAIL_MAX_LINES

  const rows = detail.kind === 'diff'
    ? [
        ...detail.removed.map(text => ({ text, tone: 'removed' as const })),
        ...detail.added.map(text => ({ text, tone: 'added' as const })),
      ].slice(0, INLINE_DETAIL_MAX_LINES)
    : detail.lines
        .slice(0, INLINE_DETAIL_MAX_LINES)
        .map(text => ({ text, tone: 'plain' as const }))

  return (
    <div className="ml-5 mb-1 pl-2 border-l-2 border-muted">
      <pre className="font-mono text-[11px] leading-[1.5] whitespace-pre-wrap break-words">
        {rows.map((row, i) => (
          <div
            key={i}
            className={cn(
              row.tone === 'removed' && 'text-destructive',
              row.tone === 'added' && 'text-success',
              row.tone === 'plain' && 'text-muted-foreground'
            )}
          >
            {row.tone === 'removed' ? `- ${row.text}` : row.tone === 'added' ? `+ ${row.text}` : row.text}
          </div>
        ))}
      </pre>
      {hidden > 0 && (
        <button
          ...
          className={cn(
            ...
            onOpenDetails && 'hover:text-foreground hover:underline'
          )}
        >
          {i18n.t('turnCard.moreLines', { count: hidden })}
        </button>
      )}
    </div>
  )
...
function ActivityRow({ activity, onOpenDetails, isLastChild, sessionFolderPath, displayMode = 'detailed', onOpenFile, onOpenUrl }: ActivityRowProps) {
  const depth = activity.depth || 0
  const [isExpanded, setIsExpanded] = useState(false)

  // Reasoning and commentary share one row type - see TextActivityRow
  if (activity.type === 'thinking' || activity.type === 'intermediate') {
    return (
      <TextActivityRow
        activity={activity}
        kind={activity.type}
  ...
  const inlineDetail = isComplete ? getInlineToolDetail(activity) : null
      ...
      <AnimatePresence initial={false}>
        {isExpanded && inlineDetail && (
          <motion.div
            ...
            className="overflow-hidden"
          >
            <InlineToolDetail detail={inlineDetail} onOpenDetails={onOpenDetails} />
          </motion.div>
        )}
      </AnimatePresence>
      </div>
 ...
 */
function ActivityGroupRow({ group, expandedGroups: externalExpandedGroups, onExpandedGroupsChange, onOpenActivityDetails, animationIndex = 0, sessionFolderPath, displayMode = 'detailed', onOpenFile, onOpenUrl }: ActivityGroupRowProps) {
...
/**
 * ResponseCard - Expanded AI response / plan content (no clipped card chrome)
 *
 * Variants:
...
export function ResponseCard({
  ...
  useEffect(() => {
    ...
    // Capture-phase: scroll events don't bubble. Window capture catches the
    // chat scroller; root capture still covers nested overflow surfaces
    // (MarkdownDocBlock, mermaid, tables).
    window.addEventListener('scroll', scheduleCoordsRecompute, { capture: true, passive: true })
      ...
      <>
        <div className="relative group">
          ...
          <button
            ...
            className={cn(
              "absolute top-0 right-0 p-1 rounded-[6px] transition-all z-10 select-none",
          ...
          {isPlan && (
            <div
              className={cn(
                "mb-2 flex items-center gap-2 select-none",
                SIZE_CONFIG.fontSize
              )}
            >
          ...
          {/* Compact Accept Plan only (mobile / auto-compact / popover).
...
export const TurnCard = React.memo(function TurnCard({
  ...
  const hasNoMeaningfulWork = activities.length > 0
    && activities.every(a => {
      ...
      // Intermediate and reasoning activities must have no meaningful content
