...
describe('turn lifecycle scenarios', () => {
  ...
  describe('merge consecutive finals in one user turn', () => {
    it('concatenates several stopReason=stop assistant texts around tools into one response', () => {
      resetCounters()

      const messages: Message[] = [
        createUserMessage('抓取 HN 头条'),
        createAssistantMessage(false, false, 'turn-a', '我先阅读浏览器工具文档，然后打开 Hacker News 抓取头条新闻。'),
        createToolMessage('completed', 'Read', 'turn-b'),
        createAssistantMessage(false, false, 'turn-c', '文档已读。现在打开浏览器并访问 Hacker News。'),
        createToolMessage('completed', 'Browser', 'turn-d'),
        createAssistantMessage(false, false, 'turn-e', '已成功抓取首页数据。'),
      ]
      ...
      expect(turn.response?.text).toBe([
        '我先阅读浏览器工具文档，然后打开 Hacker News 抓取头条新闻。',
        '文档已读。现在打开浏览器并访问 Hacker News。',
        '已成功抓取首页数据。',
      ].join('\n\n'))
    ...
    it('still splits into a new AI message after the next user question', () => {
      resetCounters()

      const messages: Message[] = [
        createUserMessage('第一问'),
        createAssistantMessage(false, false, 'turn-a', '第一答'),
        createUserMessage('第二问'),
        createAssistantMessage(false, false, 'turn-b', '第二答'),
      ]

      const turns = groupMessagesByTurn(messages)
      const assistantTurns = turns.filter(t => t.type === 'assistant') as AssistantTurn[]
      expect(assistantTurns).toHaveLength(2)
      expect(assistantTurns[0]?.response?.text).toBe('第一答')
      expect(assistantTurns[1]?.response?.text).toBe('第二答')
    })
  })
