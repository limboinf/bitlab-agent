...
describe('shouldShowThinkingIndicator', () => {
  it('returns true for pending phase', () => {
    expect(shouldShowThinkingIndicator('pending')).toBe(true)
  })

  it('returns true for awaiting phase (the gap)', () => {
    expect(shouldShowThinkingIndicator('awaiting')).toBe(true)
  })

  it('returns false for tool_active phase (tools are visible)', () => {
    expect(shouldShowThinkingIndicator('tool_active')).toBe(false)
  })

  it('returns false while streaming - the growing text is the feedback', () => {
    expect(shouldShowThinkingIndicator('streaming')).toBe(false)
  })

  it('returns false for complete phase', () => {
    expect(shouldShowThinkingIndicator('complete')).toBe(false)
  })
})
