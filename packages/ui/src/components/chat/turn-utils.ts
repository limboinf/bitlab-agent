 ...
 * A streaming response needs no indicator — the text itself is the feedback.
 *
...
function joinResponseText(existing: string, next: string): string {
 ...
 * - Final assistant messages stay on the current turn and are concatenated
 *   (one user question → one AI message, even if the model emitted several
 *   stopReason=stop texts around tool calls)
 ...
 * a user message and the next user message should be ONE turn.
 */
export function groupMessagesByTurn(messages: Message[], options: GroupTurnsOptions = {}): Turn[] {
  ...
  for (const message of sortedMessages) {
    ...
    if (message.role === 'tool') {
      ...
      // More work arrived after a previous "final" text — reopen the turn
    ...
    if (message.role === 'assistant') {
      // Only messages already known to be intermediate become activities.
      // Pending text (still streaming, isIntermediate not yet decided) falls
      // through to the response branch so the answer grows in the chat column
      // instead of scrolling by as a one-line activity. If text_complete later
      // reports isIntermediate, this same function re-runs over the updated
      // message and files it under activities — the reclassification is free
      // because grouping is a pure function of the message list.
      if (message.isIntermediate) {
        ...
        const intermediateActivity: ActivityItem = {
          id: message.id,
          // Reasoning renders as its own step type, never as commentary
      ...
      if (currentTurn.response) {
        currentTurn.response = {
          ...incoming,
          text: joinResponseText(currentTurn.response.text, incoming.text),
          // Last message id so branch-from-this-message keeps later fragments
          messageId: incoming.messageId,
          annotations: incoming.annotations ?? currentTurn.response.annotations,
        }
      } else {
        currentTurn.response = incoming
      }
