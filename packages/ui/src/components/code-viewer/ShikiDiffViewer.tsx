...
import { registerBitlabShikiThemes } from './registerShikiThemes'
...
export interface ShikiDiffViewerProps {
   ...
   *  Shiki theme natively. Falls back to bitlab-dark/bitlab-light (transparent bg) if not set. */
  shikiTheme?: string
...
export function ShikiDiffViewer({
  ...
  // to bitlab-dark/bitlab-light which have transparent bg for CSS variable theming
  const resolvedThemeName = shikiTheme || (theme === 'dark' ? 'bitlab-dark' : 'bitlab-light')
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      ;(header as any).__bitlabClickCleanup = () => header.removeEventListener('click', handleClick)
    }, 150)
    ...
    return () => {
      ...
      const header = diffsContainer?.shadowRoot?.querySelector('[data-diffs-header]')
      if (header) {
        ;(header as any).__bitlabClickCleanup?.()
      }
    }
