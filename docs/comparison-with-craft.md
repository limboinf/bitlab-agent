# Bitlab vs. Craft Agents — current comparison

> Snapshot taken on **2026-08-10** against Bitlab `main` at `00c0df4` and the upstream tag [`craft-ai-agents/craft-agents-oss` `v0.11.2` / `a60ebc1a5a7c`](https://github.com/craft-ai-agents/craft-agents-oss). Numbers are repository snapshots, not live release metrics. Re-run the commands in the final section after changing either baseline; the current checkout has a pre-existing `README.md` lineage-manifest warning if that file is modified without refreshing the manifest.

This document explains, with evidence, what Bitlab keeps from Craft Agents, what it physically removes, and how those choices change the artifact you ship. Bitlab is the "Lite" derivative built on the same architecture and renderer; the table below is the canonical answer to "what's actually different?".

## 1. Repository & source line count

Both repositories are Bun monorepos with the same workspace layout (`apps/{electron,webui,cli}` + `packages/{core,shared,ui,server-core,server,pi-agent-server,session-tools-core}`). Bitlab reuses that layout, drops two product-only packages from Craft (`messaging-gateway`, `messaging-whatsapp-worker`), removes the entire `apps/viewer` app, and never instantiates Craft's session-MCP/bridge-MCP servers.

| Metric | Bitlab | Craft Agents | Notes |
|---|---:|---:|---|
| Tracked TypeScript / TSX LOC (`*.ts`,`*.tsx`, excludes `node_modules`,`dist`,`release`,`.git`) | **190,558** | 344,964 | Bitlab source ≈ **55 %** of Craft's |
...
`SESSION_TOOL_DEFS` in `packages/session-tools-core/src/tool-defs.ts` is the single source of truth. The model sees every entry here with the `mcp__session__` prefix. Bitlab keeps **15** of these; Craft exposes **27**.

| Tool (model-visible name) | Bitlab | Craft | What it does / why Bitlab dropped it |
|---|:---:|:---:|---|
...
| `mcp__session__set_session_labels` | ❌ | ✅ | User-configurable labels on a session. Bitlab has no labels product area. |
| `mcp__session__set_session_status` | ❌ | ✅ | User-configurable status on a session. Bitlab has no user statuses. |
...
Craft Agents' second registered backend, `claude-agent-sdk`, brings Claude-Code-style tools that Pi does not define. Bitlab has no Claude backend, so none of these exist in Bitlab. The Claude backend binding is physically removed — `packages/shared/src/agent/backend/internal/drivers/` does not carry a `claude-agent-sdk` driver — and even if a tool name appeared in the system prompt, Bitlab has no execution path for it.

| Tool | Backend | Bitlab | Craft |
|---|---|:---:|:---:|
...
These are the sizes you actually ship to users, taken from the on-disk dev build at `apps/electron/release/<arch>/Bitlab.app` and the upstream `craft-agents-oss` checkout that the audit script read. **None** of these include code-signing overhead (Bitlab dev build sets `BITLAB_DEV_RUNTIME=1`; release builds with `CSC_IDENTITY_AUTO_DISCOVERY=false` are unsigned/ad-hoc). For a Craft Agents reference, the `claude-agent-sdk-darwin-arm64/claude` binary alone (`217 MB`) was measured directly from `node_modules`.

### 4.1 macOS arm64 (`Bitlab.app` / `Craft-Agents-arm64.app`)

| Component | Bitlab | Craft Agents | Delta (Craft − Bitlab) |
|---|---:|---:|---:|
...
| **`Bitlab.app` total (unpacked)** | **438 MB** | **~907 MB** | **~−469 MB** |

> The unpacked `.app` sizes already include helper apps and the Electron framework; they **exclude** the platform download (DMG/ZIP wrapper). Because `craft-agents-oss/node_modules/@anthropic-ai/claude-agent-sdk-darwin-arm64/claude` is a single **217 MB** binary with a per-platform `.zip` for `-darwin-x64` / `-win32-x64` / `-linux-x64` of similar shape, the upstream DMG/ZIP for Craft always exceeds Bitlab's by **≥ ~250 MB** after compression.

### 4.2 What `electron-builder` carries in `extraResources`

| Bundled to installer | Bitlab | Craft Agents | Approx. size carried in installer |
|---|---|---|---:|
| Per-platform **`claude` native binary** (Anthropic SDK) | ❌ | ✅ | **~217 MB per platform arch** |
| Bundled **`uv`** Python launcher (target-platform binary under `resources/bin/<platform-arch>/`) | ✅ (`uv 0.10.6`; injected through `BITLAB_UV`) | ✅ | ~30–55 MB for the target arch in each package |
...
| Network proxy | ✅ | ✅ |
| Auto-update via `electron-updater` against GitHub Releases | ✅ (against `limboinf/bitlab-agent`) | ✅ (against `https://agents.craft.do/electron/latest`) |
...
| Gate | Bitlab | Craft Agents | Result |
|---|---|---|---|
...
| `bun run typecheck:all` | passes; `apps/online-docs` is excluded from the workspace by `workspaces` globs in Bitlab and skipped in Craft | passes | both green |
| `bun run lint` | `lint:electron`, `lint:shared`, `lint:ui` pass; **20 React Hook `exhaustive-deps` warnings retained** from upstream | adds `lint:ipc-sends`, `lint:tool-name-checks`, `lint:i18n:coverage`, `lint:i18n:strings`; **45 Craft-origin React Hook warnings** | Bitlab's lint scope is narrower |

The source-lineage audit that produced these figures (`audit:craft-reuse`, `lint:craft-*`, and the sha256 override manifests) was removed during the Bitlab rename; the numbers above are a point-in-time snapshot against the pinned Craft baseline.

## 8. License & attribution

Both projects are released under **Apache-2.0**. Bitlab ships [`NOTICE`](../NOTICE) on the repo root with the attribution upstream required, and [`docs/featues.md`](./featues.md) records the kept/removed capabilities in human-readable form. Source and release artifacts (DMG/ZIP/NSIS/AppImage, manifests, blockmaps, and checksums) now share the `limboinf/bitlab-agent` repository; no release-only mirror is used.

## 9. Re-running this audit

```bash
# From the Bitlab checkout
git rev-parse HEAD              # record Bitlab commit
