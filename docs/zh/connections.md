...
这两个字段来自 `packages/shared/src/config/llm-connections.ts` 中的 `LlmConnection`,共同决定 Bitlab 如何连上一个模型。
...
| `customEndpoint.supportsThinking` | `true`、`false`(默认) | 请求里是否带上由会话思考等级换算出的 reasoning-effort 参数。默认关闭,因为不认识该参数的端点会直接拒绝整个请求。可在单个模型条目上用 `supportsThinking` 覆盖。 |
...
私有网关上的推理模型 —— 连接级开启推理请求,其中一个纯文本模型单独关掉:
...
端点**返回**的推理内容(`reasoning_content` 等字段)无论这个开关如何都会被解析并渲染 —— `supportsThinking` 只控制 Bitlab 主动请求什么。
