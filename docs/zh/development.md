...
本节汇总如何搭建 Bitlab 的开发环境,以及每个 release 产物对应的命令。
...
默认配置根目录为 `~/.bitlab`,但为了并行开发或隔离测试可以重定向:

```bash
BITLAB_CONFIG_DIR=/tmp/bitlab-dev bun run server:dev:webui
```

`BITLAB_CONFIG_DIR` 在模块加载时(`packages/shared/src/config/paths.ts`)读一次,所有下游路径(workspaces、credentials、logs、tool icons)都会跟随。测试通过 `BITLAB_CONFIG_DIR` 显式注入,不会在 `$HOME` 下创建文件。

## 常用环境变量

| 变量 | 默认值 | 作用 |
|---|---|---|
| `BITLAB_CONFIG_DIR` | `~/.bitlab` | 配置根(也叫数据目录)覆盖入口 |
| `BITLAB_SERVER_TOKEN` | — | headless server RPC 鉴权必需的 bearer token |
| `BITLAB_RPC_HOST` / `BITLAB_RPC_PORT` | `127.0.0.1` / `9100` | server 绑定地址 / 端口 |
| `BITLAB_RPC_TLS_CERT` / `_KEY` / `_CA` | — | 启用 `wss://`,使用 PEM 编码的 cert/key;CA 可选 |
| `BITLAB_HEALTH_PORT` | `0`(关闭) | 绑定 sidecar HTTP health 端点 |
| `BITLAB_APP_ROOT` | dev 下为 repo 根 | server 读取打包资源的根 |
| `BITLAB_RESOURCES_PATH` | 同 app root | 覆盖 resources 目录 |
| `BITLAB_BUNDLED_ASSETS_ROOT` | 不设置 | dev 用覆盖,把 headless server 指向 `apps/electron` resources |
| `BITLAB_IS_PACKAGED` | `false` | 生产构建内设置为 `true` |
| `BITLAB_VERSION` | `package.json#version` | 覆盖 server 上报的版本 |
| `BITLAB_DEBUG` | 不设置 | 启用额外 debug 日志 |
| `BITLAB_WEBUI_DIR` | 不设置 | 在 RPC 端口上启用 WebUI 资源 |
| `BITLAB_WEBUI_PASSWORD` / `_SECURE_COOKIE` / `_WS_URL` | 不设置 | WebUI 登录密码、cookie `Secure` 标志覆盖、浏览器侧 `ws://` URL |
...
这一组合在全新 checkout 上通过了 Bitlab 完整门禁(见 `migration/migration-features.md` "最终验证结果")。该门禁不启动 GUI,不需要录音/截屏权限。
