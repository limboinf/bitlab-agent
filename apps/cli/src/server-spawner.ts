/**
 * Server spawner — start a headless Bitlab server as a child process.
 *
 * Spawns `bun run <serverEntry>`, reads stdout for the `BITLAB_SERVER_URL=`
 * and `BITLAB_SERVER_TOKEN=` lines, and returns a handle to stop the server.
 */
...
export async function spawnServer(opts?: SpawnServerOptions): Promise<SpawnedServer> {
  ...
  // Read stdout line by line looking for BITLAB_SERVER_URL=
  return new Promise<SpawnedServer>((resolve, reject) => {
    ...
    const processLines = () => {
      ...
      for (const line of lines) {
        ...
        if (line.startsWith('BITLAB_SERVER_TOKEN=')) {
    ...
    ;(async () => {
      ...
      clearTimeout(timer)
      if (!url) {
        reject(new Error('Server process exited before printing BITLAB_SERVER_URL'))
      }
    })()
  })
}
