...
This guide explains how to customize the visual theme of Bitlab.

## Overview

Bitlab uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.bitlab/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.bitlab/theme.json` - Override specific colors (app-level)
...
```
~/.bitlab/workspaces/{id}/config.json
```
...
Create `~/.bitlab/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.bitlab/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.bitlab/themes/{name}.json`
...
1. Create `~/.bitlab/theme.json` for overrides or `~/.bitlab/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.bitlab/theme.json` for overrides, `~/.bitlab/themes/` for presets)
