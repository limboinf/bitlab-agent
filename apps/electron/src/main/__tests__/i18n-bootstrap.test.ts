 ...
 *
 * `BITLAB_CONFIG_DIR` is captured at module-load, so each scenario runs in a
 * subprocess with `BITLAB_CONFIG_DIR` set in its env (same pattern as
    ...
    try {
      const r = runScript(
        ...
        `
          ...
          import { setPersistedUiLanguage, getPersistedUiLanguage } from '@bitlab/shared/config';
          setupI18n();
  ...
  it('returns undefined when no language is persisted (no hydration step)', () => {
    ...
    try {
      const r = runScript(
        ...
        `
          import { setupI18n, i18n } from '@bitlab/shared/i18n';
          import { getPersistedUiLanguage } from '@bitlab/shared/config';
          setupI18n();
          const persisted = getPersistedUiLanguage();
