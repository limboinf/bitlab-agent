 ...
 * Deep Link Handler
 *
 * Parses bitlab:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   bitlab://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   bitlab://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   bitlab://archived[/session/{sessionId}]            - Archived sessions
 *   bitlab://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   bitlab://action/{actionName}[/{id}][?params]
 *   bitlab://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *
 * Examples:
 *   bitlab://allSessions                               (all sessions view)
 *   bitlab://allSessions/session/abc123                (specific session)
 *   bitlab://settings/shortcuts                     (shortcuts page)
 *   bitlab://action/new-chat                        (uses active window)
 *   bitlab://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
import type { EventSink } from '@bitlab/server-core/transport'
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== 'bitlab:') {
      return null
    }

    // For custom protocols, the hostname contains the first path segment
    // e.g., bitlab://workspace/ws123 → hostname='workspace', pathname='/ws123'
    // e.g., bitlab://allSessions/chat/abc → hostname='allSessions', pathname='/chat/abc'
    ...
    // bitlab://allSessions/..., bitlab://settings/..., etc. (compound routes)
    ...
    // bitlab://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // bitlab://action/... (no workspace - uses active window)
    if (host === 'action') {
