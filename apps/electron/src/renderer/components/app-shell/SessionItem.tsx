...
export function SessionItem({
  ...
  // Same precedence as the status gutter below — the tooltip must name whichever
  // marker actually rendered, not the first condition that happens to be true.
  const statusLabel = item.isProcessing
    ? t('sessionStatus.running')
    : hasPendingPrompt
      ? t('sessionStatus.awaitingApproval')
      : item.lastMessageRole === 'plan'
        ? t('sessionStatus.planReady')
        : hasUnreadMeta(item)
          ? t('sessionStatus.unread')
          : undefined

  // Flag sits beside the title, not in the trailing slot — it used to share that
  // slot with the timestamp, so flagging a session silently hid when it last ran.
  const flagSuffix = item.isFlagged ? (
    <Flag className="h-3 w-3 text-info" aria-label={t('sessionStatus.flagged')} />
  ) : undefined
  ...
  // No per-row rule (`showSeparator={false}`): at compact density the rows are
  // ~32px, and a line between every one of them reads as noise. Hover and
  // selection tint carry the separation; date group headers chunk the list.
  return (
    <EntityRow
      ...
      icon={(
        // Fixed-width status gutter: always occupies space so every title starts
        // at the same x, and a session flipping to "processing" doesn't shove the
        // whole list sideways. One status at a time, highest urgency wins.
        <div
      ...
      titleClassName={cn('text-xs', item.isAsyncOperationOngoing && 'animate-shimmer-text')}
      ...
      titleTrailing={hasMatch ? (
        ...
        <span className="text-[11px] text-foreground/35 tabular-nums whitespace-nowrap">
