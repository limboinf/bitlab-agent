 ...
 * - Project location: Default (~/Bitlab Workspaces/) or Custom
 */
export function AddWorkspaceStep_CreateNew({
  ...
  const defaultBasePath = homeDir ? `${homeDir}/Bitlab Workspaces` : null
  ...
  const handleCreate = useCallback(async () => {
    if (!name.trim() || !finalPath) return
    await onCreate(finalPath, name.trim())
  }, [name, finalPath, onCreate])

  const canCreate = name.trim() && finalPath && !isCreating
