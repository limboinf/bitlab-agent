...
import { usePlatform } from '@bitlab/ui'
...
const EDIT_CONFIGS: Record<EditContextKey, (location: string) => EditConfig> = {
  ...
  'default-permissions': (location) => ({
    context: {
      ...
      context:
        'The user is editing app-level default permissions (~/.bitlab/permissions/default.json). ' +
  ...
  'skill-metadata': (location) => ({
    context: {
      ...
      context:
        ...
        'Frontmatter follows the Agent Skills specification: name (required), description (required — say what it does AND when to use it), ' +
        'license (optional), compatibility (optional), metadata (optional string map; Bitlab keys are namespaced, e.g. bitlab.icon for an emoji or URL). ' +
  ...
  'preferences-notes': (location) => ({
    context: {
      ...
      context:
        'The user is editing the notes field in their preferences (~/.bitlab/preferences.json). ' +
  ...
  'add-skill': (location) => ({
    context: {
      ...
      context:
        ...
        'Follow the patterns in ~/.bitlab/docs/skills.md. ' +
  ...
  'edit-tool-icons': (location) => ({
    context: {
      ...
      context:
        ...
        'The file is tool-icons.json in ~/.bitlab/tool-icons/. Icon image files live in the same directory. ' +
        ...
        'Read ~/.bitlab/docs/tool-icons.md for full format reference. ' +
 ...
 *
 * @example
 * const { context, example } = getEditConfig('workspace-permissions', workspace.dataRoot)
 */
...
export function EditPopover({
  ...
  const handleLegacySendMessage = useCallback((message: string) => {
    ...
    const url = `bitlab://action/new-session?input=${encodedInput}&send=true&mode=${permissionMode}&badges=${encodedBadges}${modelParam}${systemPromptParam}`
