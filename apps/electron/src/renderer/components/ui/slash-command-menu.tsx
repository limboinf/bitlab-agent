...
import {
  ...
  type PermissionMode,
} from '@bitlab/shared/agent/modes'
...
/** Only modes the workspace actually exposes should be selectable. */
function enabledPermissionModeCommands(enabledModes: PermissionMode[]): SlashCommand[] {
  return permissionModeCommands.filter(cmd => enabledModes.includes(cmd.id as PermissionMode))
}

export function buildPermissionModeGroups(enabledModes: PermissionMode[]): CommandGroup[] {
  return [{ id: 'modes', commands: enabledPermissionModeCommands(enabledModes) }]
}
...
function flattenSections(sections: SlashSection[]): SlashCommand[] {
...
function isPermissionModeId(id: SlashCommandId): id is PermissionMode {
  return id in PERMISSION_MODE_LABEL_KEYS
}

function CommandItemContent({ command, isActive }: { command: SlashCommand; isActive: boolean }) {
  const { t } = useTranslation()
  const label = isPermissionModeId(command.id)
    ? t(PERMISSION_MODE_LABEL_KEYS[command.id].label, command.label)
    : command.label
  return (
    <>
...
export function InlineSlashCommand({
  ...
  const handleSelect = React.useCallback((item: SlashCommand) => {
  ...
  const menuStyle = getInlineMenuFixedStyle(
    position,
  ...
  const menu = (
    <div
      ...
      style={{ left: menuStyle.left, bottom: menuStyle.bottom, minWidth: 220, maxWidth: 260 }}
    >
...
export function useInlineSlashCommand({
  ...
  const sections = React.useMemo((): SlashSection[] => {
    ...
    // Modes section
    const modeItems = enabledPermissionModeCommands(enabledModes)
