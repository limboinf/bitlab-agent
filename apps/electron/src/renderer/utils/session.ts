 ...
 *  Human-readable compact strings: "刚刚", "15分钟前", "2小时前", …
 *  Uses i18n keys (time.compact.*) so output is localized. */
export const shortTimeLocale = {
  formatDistance: (token: string, count: number) => {
    // Sub-minute ages always read as "just now" — "0秒/3秒" is noise in a session list.
    if (token === 'xSeconds' || (token === 'xMinutes' && count < 1)) {
      return i18next.t('time.compact.justNow')
    }
