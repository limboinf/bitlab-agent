...
import type { SessionDraft, DraftAttachmentRef } from '@bitlab/shared/config'
...
import { TooltipProvider } from '@bitlab/ui'
...
import { extractWorkspaceSlugFromPath } from '@bitlab/shared/utils/workspace-slug'
import { DEFAULT_THINKING_LEVEL } from '@bitlab/shared/agent/thinking-levels'
...
export default function App() {
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        if (needs.isFullyConfigured) {
          // Bitlab is local-first. If a window has no explicit workspace,
  ...
  // Platform actions for @bitlab/ui components (overlays, etc.)
  ...
  const platformActions = useMemo(() => ({
    ...
    // Hand an HTML file to the OS so it opens in the user's browser. Goes through
    // shell.openPath (the same validated OPEN_FILE channel) rather than
    // shell.openExternal on a file:// URL, which is blocked repo-wide.
    onOpenFileInBrowser: linkInterceptor.openFileExternal,
    // URL the HTML preview iframe loads from — see html-preview-protocol.ts for
    // why the preview can't just inline the source.
    getHtmlPreviewUrl: buildHtmlPreviewUrl,
