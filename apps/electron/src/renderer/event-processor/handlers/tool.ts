...
import { findToolStartTargetIndex, findOpenToolMessageIndex, findLatestToolMessageIndex } from '@bitlab/core'
import {
  updateMessageAt,
  appendMessage,
  generateMessageId
 ...
 *
 * A finished call never matches: providers that reuse tool call ids across
 * rounds must not overwrite an earlier, already-completed call.
 */
export function handleToolStart(
  ...
  const existingIndex = findToolStartTargetIndex(session.messages, event.toolUseId)
...
export function handleToolResult(
  ...
  // Pair with the call in flight — a repeated tool call id must not overwrite
  // the result of an earlier, already-finished call.
  const toolIndex = findOpenToolMessageIndex(session.messages, event.toolUseId)
  ...
  // without a prior tool_start. If tool_start arrives later it won't match this
  // message (it is already finished) and starts its own occurrence instead.
...
export function handleTaskBackgrounded(
  ...
  const toolIndex = findLatestToolMessageIndex(session.messages, event.toolUseId)
