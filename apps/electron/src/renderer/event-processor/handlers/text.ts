...
export function handleTextComplete(
  ...
  if (msgIndex !== -1) {
    ...
    // A completed intermediate message is done — never reuse its slot, whatever
    // arrives next. Each thinking block (e.g. Codex reasoning between tool calls)
    // stays its own message, and a final text must not swallow the commentary
    // that preceded it when both carry the same turnId.
