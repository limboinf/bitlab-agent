...
export function findStreamingMessage(
  ...
  if (turnId) {
    const index = messages.findIndex(m =>
      m.role === 'assistant' && !m.isThinking && m.turnId === turnId && m.isStreaming
    )
    if (index !== -1) return index
  }
  // Fallback: find last streaming assistant message.
  // Reasoning messages are excluded — they stream concurrently with answer
  // text on some providers, and the fallback would append the answer onto
  // the open reasoning block.
  for (let i = messages.length - 1; i >= 0; i--) {
    if (messages[i].role === 'assistant' && !messages[i].isThinking && messages[i].isStreaming) {
      return i
    }
  }
  return -1
...
export function findAssistantMessage(
  ...
  if (turnId) {
    const index = messages.findIndex(m =>
      m.role === 'assistant' && !m.isThinking && m.turnId === turnId
    )
    if (index !== -1) return index
  }
  // Fallback: find last streaming assistant message (never a reasoning block —
  // see findStreamingMessage)
  for (let i = messages.length - 1; i >= 0; i--) {
    if (messages[i].role === 'assistant' && !messages[i].isThinking && messages[i].isStreaming) {
      return i
    }
