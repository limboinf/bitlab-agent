...
const ChatPage = React.memo(function ChatPage({ sessionId }: ChatPageProps) {
  ...
  // Changing permissions from the composer applies to this session right away and
  // is remembered as the workspace default, so the next session starts where the
  // user left off instead of silently reverting.
  const handlePermissionModeChange = React.useCallback((mode: PermissionMode) => {
    setPermissionMode(mode)
    if (!activeWorkspaceId) return
    void window.electronAPI?.updateWorkspaceSetting(activeWorkspaceId, 'permissionMode', mode)
      .catch((error: unknown) => {
        console.error('Failed to persist workspace permission mode:', error)
      })
  }, [setPermissionMode, activeWorkspaceId])
  ...
  const workingDirectory = activeWorkspace?.folderPath ?? activeWorkspace?.dataRoot

  const handleOpenFile = React.useCallback(
    async (path: string) => {
      // Resolve bare relative paths against the folder bound to the workspace.
      const resolved = (() => {
  ...
  const handleOpenInNewWindow = React.useCallback(async () => {
    ...
    const url = `bitlab://${route}${separator}window=focused`
    try {
  ...
  // Empty desktop chat: hide panel chrome so the capability-discovery empty
  // state can own the canvas (no orphaned "新聊天 ⌄ / ✕" floating above void).
  // Compact keeps the header for back navigation; multi-panel keeps close via rightSidebarButton.
  const isEmptyDesktopChat = !isCompactMode
    && (session.messages?.length ?? 0) === 0
    && !messageLoadState.messagesLoading
    && !( (session.messageCount ?? 0) > 0 || (sessionMeta?.messageCount ?? 0) > 0 )

  return (
    <>
      <div className="h-full flex flex-col">
        {(!isEmptyDesktopChat || rightSidebarButton || leadingAction) && (
          <PanelHeader
            title={displayTitle}
            titleMenu={titleMenu}
