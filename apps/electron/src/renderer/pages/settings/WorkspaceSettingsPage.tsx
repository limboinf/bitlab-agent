...
export default function WorkspaceSettingsPage() {
  ...
  // Mode cycling state
  const [enabledModes, setEnabledModes] = useState<PermissionMode[]>(DEFAULT_CYCLABLE_PERMISSION_MODES)
            ...
            <SettingsSection title={t("settings.workspace.permissionsSection")}>
              <SettingsCard>
                <SettingsMenuSelectRow
                  ...
                  // Only modes the composer actually exposes — enable more below.
                  options={PERMISSION_MODE_ORDER.filter((m) => enabledModes.includes(m)).map((m) => ({
                    value: m,
                    label: t(PERMISSION_MODE_LABEL_KEYS[m].label),
                    description: t(PERMISSION_MODE_LABEL_KEYS[m].description),
                  }))}
                />
              </SettingsCard>
            </SettingsSection>
