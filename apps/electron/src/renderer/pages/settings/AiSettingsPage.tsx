...
import { Spinner, FullscreenOverlayBase } from '@bitlab/ui'
...
import { dedupeModelsById } from '@/components/app-shell/input/model-picker-helpers'
...
function getModelOptionsForConnection(
  ...
  // list can repeat an id when the setup wizard's tiers collapse onto the same
  // model, which would render duplicate (and duplicately checked) options.
  if (connection.models && connection.models.length > 0) {
    return dedupeModelsById(connection.models).map((m) => {
      if (typeof m === 'string') {
