...
VERSIONS_URL="https://github.com/limboinf/bitlab-agent/releases/latest/download"
DOWNLOAD_DIR="$HOME/.bitlab/downloads"
...
if [ -z "$filename" ]; then
    filename="Bitlab-${version}-${arch}.${ext}"
fi
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="app.bitlab.desktop"
    if pgrep -x "Bitlab" >/dev/null 2>&1; then
        info "Quitting Bitlab..."
        ...
        if pgrep -x "Bitlab" >/dev/null 2>&1; then
            ...
            pkill -9 -x "Bitlab" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  Bitlab has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'Bitlab'${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.bitlab/app"
    WRAPPER_PATH="$INSTALL_DIR/bitlab"
    APPIMAGE_INSTALL_PATH="$APP_DIR/Bitlab-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "Bitlab.*AppImage" >/dev/null 2>&1; then
        info "Stopping Bitlab..."
        pkill -f "Bitlab.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# Bitlab launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.bitlab/app/Bitlab-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@bitlab"
ELECTRON_CACHE_ALT="$HOME/.cache/@bitlab"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: Bitlab not found at $APPIMAGE_PATH"
    echo "Reinstall from https://github.com/limboinf/bitlab-agent/releases/latest"
    exit 1
fi
