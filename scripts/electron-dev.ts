...
// Detects instance number from folder name suffix (e.g., bitlab-1 → instance 1)
function detectInstance(): void {
  // Don't override if already set (e.g., by sourcing detect-instance.sh first)
  if (process.env.BITLAB_VITE_PORT) return;
  ...
  if (match) {
    ...
    process.env.BITLAB_INSTANCE_NUMBER = instanceNum;
    process.env.BITLAB_VITE_PORT = `${instanceNum}173`;
    process.env.BITLAB_APP_NAME = `Bitlab [${instanceNum}]`;
    process.env.BITLAB_CONFIG_DIR = join(process.env.HOME || "", `.bitlab-${instanceNum}`);
    process.env.BITLAB_DEEPLINK_SCHEME = `bitlab${instanceNum}`;
    console.log(`🔢 Instance ${instanceNum} detected: port=${process.env.BITLAB_VITE_PORT}, config=${process.env.BITLAB_CONFIG_DIR}`);
  }
}
...
function getBuildDefines(): Record<string, string> {
  return {
    "process.env.BITLAB_DEV_RUNTIME": JSON.stringify(process.env.BITLAB_DEV_RUNTIME || "1"),
  };
}

// Get environment variables for electron process
function getElectronEnv(): Record<string, string> {
  const vitePort = process.env.BITLAB_VITE_PORT || "5173";

  return {
    ...process.env as Record<string, string>,
    VITE_DEV_SERVER_URL: `http://localhost:${vitePort}`,
    BITLAB_CONFIG_DIR: process.env.BITLAB_CONFIG_DIR || "",
    BITLAB_APP_NAME: process.env.BITLAB_APP_NAME || "Bitlab",
    BITLAB_DEEPLINK_SCHEME: process.env.BITLAB_DEEPLINK_SCHEME || "bitlab",
    BITLAB_INSTANCE_NUMBER: process.env.BITLAB_INSTANCE_NUMBER || "",
  };
}

function getElectronCommand(): string[] {
  const configDir = process.env.BITLAB_CONFIG_DIR;
  const userDataArg = process.env.BITLAB_INSTANCE_NUMBER && configDir
    ? [`--user-data-dir=${join(configDir, "electron-user-data")}`]
    : [];
  const remoteDebuggingArg = process.env.BITLAB_REMOTE_DEBUGGING_PORT
    ? [`--remote-debugging-port=${process.env.BITLAB_REMOTE_DEBUGGING_PORT}`]
    : [];
...
// `require()`, so Electron-specific top-level requires
