...
$DOWNLOAD_DIR = "$env:TEMP\bitlab-install"
$APP_NAME = "Bitlab"
...
if (-not $filename) {
    $filename = "Bitlab-$version-$arch.exe"
}
...
$process = Get-Process -Name "Bitlab" -ErrorAction SilentlyContinue
if ($process) {
    Write-Info "Closing Bitlab..."
...
Write-Info "Adding 'bitlab' command to PATH..."

$binDir = "$env:LOCALAPPDATA\Bitlab\bin"
$cmdFile = "$binDir\bitlab.cmd"
$exePath = "$env:LOCALAPPDATA\Programs\Bitlab\Bitlab.exe"
...
if ($userPath -notlike "*$binDir*") {
    ...
    Write-Success "Added to PATH (restart terminal to use 'bitlab' command)"
} else {
    Write-Success "Command 'bitlab' is ready"
}
...
Write-Host ""
Write-Host "  Bitlab has been installed."
Write-Host ""
Write-Host "  Launch from:"
Write-Host "    - Start Menu or desktop shortcut"
Write-Host "    - Command line: bitlab (restart terminal first)"
Write-Host ""
}
